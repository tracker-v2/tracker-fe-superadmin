    ...
    <SidebarGroup className='mt-5'>
      <SidebarMenu className={isCollapsed ? "items-center" : ""}>
        {/* {items.map((item, index) => (
          <Collapsible key={item.title} asChild defaultOpen={item.isActive} className={`group/collapsible ${index > 0 ? "mt-1" : ""}`}>
            <SidebarMenuItem>
              <CollapsibleTrigger asChild className='gap-2 p-5 items-center'>
                <SidebarMenuButton
                  tooltip={item.title}
                  className={`${isCollapsed ? "items-center" : ""} cursor-pointer ${
                    pathname === item.url ? "bg-blue-900 text-primary-foreground" : item.url === "/perawatan-kendaraan" && pathname === "/riwayat-kendaraan" ? "bg-blue-900 text-primary-foreground" : ""
                  }`}

...
type DialogContentPropsWithCheckType = React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content> & {
  type?: 'right' | 'hidden' | 'default';
};
...
const DialogContent = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Content>, DialogContentPropsWithCheckType>(({ className, children, type, ...props }, ref) => (
  <DialogPortal>
    <DialogOverlay />
    ...
    >
      ...
      <DialogPrimitive.Close
        className={`rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground ${
          type === 'right' ? 'right-6 top-6' : 'right-4 top-4'
        } ${type === 'hidden' ? 'hidden' : 'absolute'}`}
      >
        <X className="h-4 w-4" />
